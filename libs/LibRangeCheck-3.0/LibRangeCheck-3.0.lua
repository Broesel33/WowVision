...
-- WowVision: Mists Classic ships a hybrid API. C_Spell.IsSpellInRange works,
-- but there is NO spellbook enumeration (neither the global GetSpellBookItemInfo
-- nor C_SpellBook.GetSpellBookItemInfo), so LibRangeCheck's findSpellIdx scan
-- resolves zero spells and only the 8/28 interact checkers survive. When the
-- spellbook can't be scanned but the modern range API exists, check candidate
-- spell IDs directly via IsSpellKnown + C_Spell.IsSpellInRange instead of
-- resolving a spellbook index. Older Classic clients still expose the global
-- GetSpellBookItemInfo, so they keep their existing behavior.
-- Evaluated lazily, NOT at file load: LibRangeCheck loads early (via libs.xml),
-- and on Mists the deprecated spellbook globals report differently then than
-- they do post-login. Re-check at checker-build time so we get the real state.
local function useSpellIDChecker()
  local canScan = _G.GetSpellBookItemInfo or (C_SpellBook and C_SpellBook.GetSpellBookItemInfo)
  return (not canScan) and C_Spell and C_Spell.IsSpellInRange and IsSpellKnown and true or false
end
...
-- WowVision: range check by spellbook index (default) or directly by spell ID
-- (Mists Classic, where the spellbook can't be enumerated). Returns 1/0/nil.
local function spellIdxInRange(spellIdx, unit)
  if useSpellIDChecker() then
    local result = C_Spell.IsSpellInRange(spellIdx, unit)
    if result == true then
      return 1
    elseif result == false then
      return 0
  ...
  return IsSpellBookItemInRange(spellIdx, BOOKTYPE_SPELL, unit)
end
...
local checkers_Spell = setmetatable({}, {
  __index = function(t, spellIdx)
    local func = function(unit)
      if spellIdxInRange(spellIdx, unit) == 1 then
        return true
      end
    end
    t[spellIdx] = func
...
local function getSpellData(sid)
  ...
  if useSpellIDChecker() then
    -- No spellbook to scan; the spell ID is usable as the checker key if known.
    local usable = (name and IsSpellKnown(sid)) and sid or nil
    return name, fixRange(minRange), fixRange(range), usable
  end
