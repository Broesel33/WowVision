...
- `nodes.hybridScrollList(builder, { scrollFrame, count, emit, key?, label?, id?, rowHeight? })` — pilots button-pool scroll frames (HybridScrollFrame and kin, like the quest log): entries enumerate from the API. The discipline, inherited from the old ProxyScrollFrame: focusing an entry ALWAYS scrolls it to a calibrated position (re-stamping the pool synchronously), then verifies by finding the button whose index matches, rolling the scroll back if none does. Pool buttons rebind as the frame scrolls — never trust an index-to-button mapping except immediately after scrolling to that index. Secure clicks bind to a frame at engage time, so drift between engage and press is guarded twice: the HOST records what each function-valued Click target resolved to at engage and, per tick, marks bindings dirty when it resolves differently (node handles re-engage the same tick; nav bindings are never touched), while the adapters' stateless `onFocusTick` re-aligns the pane whenever the focused entry scrolls out of resolve range.
...
- `nodes.attachHover(vtable, frameOrFunction)` — runs the frame's OnEnter script when the node gains focus and OnLeave when it loses it, appended after any existing hooks. This is how the game's own tooltips and highlights follow focus (the tooltip reader reads what hover produced). proxyButton and scroll rows do this automatically; attach it on hand-written vtables over Blizzard frames. Opt out with `hover = false` on proxyButton when the frame's own hover scripts write shared state: on the modern engine those scripts run as the addon and taint what they write, and Blizzard's secure code that later reads it fails on secret values (action buttons' cooldowns in combat). Such nodes pass `tooltip = { type = "Game", mode = "immediate", populate = function(tooltip, frame) ... end }` so the reader fills the game tooltip itself (for action buttons, `tooltip:SetAction(button.action)`).
...
Dropdown menus: `core/graph/dropdownMenu.lua` watches the modern Menu manager every frame; any menu that opens anywhere becomes its own stack, and each open menu level (root, then each submenu) is one PUSHED SCREEN on it, synced both ways every tick: Enter on a submenu row opens its child through the element description's `ForceOpenSubmenu` (the only path — the manager gates hover opens on hardware `IsMouseOver`, and clicks don't open submenus at all) and the watcher pushes a screen landing on its first item; Escape pops one level and the watcher closes Blizzard's deepest menu to match; Blizzard collapsing levels pops our screens. Items are the menu's real buttons (real clicks); check and radio state reads the mark texture's atlas swap; titles read as text. Menu frames attach via the Window API, so discovery is a `hooksecurefunc` on `MenuProxyMixin.OnLoad` capturing every pooled menu frame at creation. `Module:registerDropdownMenu(menuKey, emitters)` overrides specific rows: `emitters[index] = function(builder, itemFrame, index)`.
...
- `startRow(rowKey)` / `endRow()` make horizontal rows; rows sharing a non-nil key get column-preserving vertical navigation. A row of two or more controls that a sighted user reads as one thing (a setting with a second control to its right, a binding's two key slots) should sit in its own context named for the row: a context holding exactly one multi-item row announces "bar", so entering it tells the user there is something to the right. When the context's label equals its leading control's label, the level speaks the name once with the bar word and the control drops its own label ("Click to Move, bar, checkbox, unchecked, 1 of 2"); a level whose label merely prefixes a compound label ("Jump" over "Jump, Space") stays silent.
